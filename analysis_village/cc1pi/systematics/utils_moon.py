...
def get_frac_unc(df, var_config):
    var = df[var_config.var_evt_reco_col]
    ...
    # for syst in ["GENIE", "Flux"]:
        for i in range(100):
            weights = df.mc[syst]["univ_{}".format(i)]
            # set nan to 1
        ...
        this_frac_unc = np.sqrt(np.diag(cov_mat / n_cv**2))
        this_frac_unc = np.where(n_cv == 0, 0, this_frac_unc)
        frac_unc += this_frac_unc**2
    frac_unc = np.sqrt(frac_unc)
...
def get_text_color(value, cmap, norm):
    # Get the color for this specific value from the plot's normalization
    rgba = cmap(norm(value))
    
    # Compute luminance (W3C standard for perceived brightness)
    # rgba[0,1,2] are R, G, B
    luminance = 0.299 * rgba[0] + 0.587 * rgba[1] + 0.114 * rgba[2]
    
    # If the background is bright (high luminance), use black text. 
    # If dark, use white.
...
def plot_heatmap(matrix, 
    ...
    # Define uniform coordinates for the 'extent'
    ...
    im = ax.imshow(matrix, extent=extent, origin="lower", vmin=-1, vmax=1, cmap=cmap)

    # --- Power-of-10 Logic ---
    exponent = 0
    flat_matrix = matrix[~np.isnan(matrix)]
    if flat_matrix.size > 0 and np.any(flat_matrix != 0):
        example_value = np.nanmax(np.abs(flat_matrix))
        ...
        plt.colorbar(im, shrink=0.7, label=plot_labels[2])

    # --- Annotations using explicit get_text_color ---
    for i in range(nbins-1):      # rows (y)
        for j in range(nbins-1):  # columns (x)
            ...
            if not np.isnan(value):
                ...
                # Explicit call to your get_text_color function
