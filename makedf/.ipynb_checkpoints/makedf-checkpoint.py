...
TRUE_P_THRESHOLDS = {"nmu_P_100MeV_3000MeV": ["muon", 0.1, 3],
...
def make_trkdf(f, scoreCut=False, requiret0=False, requireCosmic=False, mcs=False, det="SBND", updatecalo=None):
    ...
    if updatecalo is not None:
        ...
        for plane in range(0, 3):
            trkhitdf = make_trkhitdf(f, plane, None)
            #trkhitdf = trkhitdf[InAV(df=trkhitdf, det=det)]
...
def make_trkhitdf_plane0(f, updatecalo=None):
    return make_trkhitdf(f, 0, updatecalo=updatecalo)

def make_trkhitdf_plane1(f, updatecalo=None):
    return make_trkhitdf(f, 1, updatecalo=updatecalo)

def make_trkhitdf_plane2(f, updatecalo=None):
    return make_trkhitdf(f, 2, updatecalo=updatecalo)

def make_trkhitdf(f, plane=2, updatecalo=None):
    # ----- sbnd or icarus? -----
    det = loadbranches(f["recTree"], ["rec.hdr.det"]).rec.hdr.det
    if (1 == det.unique()):
        det = "SBND"
    ...
    if updatecalo is not None:
        ...
        dedx_redo = chi2pid.dedx(df, gain=det, calibrate=det, plane=plane, isMC = ismc, new_calo_params=chi2pid.CALO_VARIATIONS[updatecalo])
        df["dedx"] = dedx_redo
...
def add_grouped_momentum(mcdf,mcprimdf,pdg,prefix='ptot'):
    group_levels=[0,1]
    
    # --- Select desired PDG ---
    sel = mcprimdf[abs(mcprimdf.pdg) == pdg]
    ...
    p_sum = (
        sel
        .groupby(level=group_levels)[[px_col, py_col, pz_col]]
        .sum()
    )
    ...
    # --- Assign outputs ---
    p_sum.loc[:, ('pmag','')] = pmag
    p_sum.loc[:, ('dir','x')] = (p_sum[px_col]/pmag).where(pmag>0, 1.0)
    p_sum.loc[:, ('dir','y')] = (p_sum[py_col]/pmag).where(pmag>0, 0.0)
    p_sum.loc[:, ('dir','z')] = (p_sum[pz_col]/pmag).where(pmag>0, 0.0)

    #print(p_sum.loc[:,[('pmag',''),('dir','x'),('dir','y'),('dir','z')]])
    # --- Keep only new columns ---
    outcols = [('pmag',''), ('dir','x'), ('dir','y'), ('dir','z')]

    for old_col in p_sum.columns:
        new_col = (prefix, old_col[0], old_col[1])
        default = 1.0 if old_col == ('dir', 'x') else 0.0
        mcdf[new_col] = p_sum[old_col].reindex(mcdf.index, fill_value=default)
...
def make_mcdf(f, branches=mcbranches, primbranches=mcprimbranches):
    ...
    # particle counts
    mcdf = multicol_add(mcdf, ((np.abs(mcprimdf.pdg)!=-1) &(np.abs(mcprimdf.pdg)!=22) & (np.abs(mcprimdf.pdg)< 1e8)).groupby(level=[0,1]).sum().rename("nprim"))
    ...
    for identifier, (particle, low_threshold, upper_threshold) in TRUE_P_THRESHOLDS.items():
        mass = PDG[particle][2]
        mask_particle = np.abs(mcprimdf.pdg) == PDG[particle][0]
        # Compute momentum on FULL df
        P = np.sqrt(mcprimdf.genp.x**2 + mcprimdf.genp.y**2 + mcprimdf.genp.z**2)
        mask_cut = (P > low_threshold) & (P < upper_threshold)
        final_mask = mask_particle & mask_cut
        mcdf = multicol_add(mcdf, final_mask.groupby(level=[0, 1]).sum().rename(identifier))
    ...
    #total p of the protons and pions summed
    mcdf =  add_grouped_momentum(mcdf,mcprimdf,2212,prefix='ptot')
    mcdf =  add_grouped_momentum(mcdf,mcprimdf,211,prefix='pitot')
...
def make_pandora_df(f, trkScoreCut=False, trkDistCut=50., cutClearCosmic=False, requireFiducial=False, updatecalo=None, **trkArgs):
    # load
    trkdf = make_trkdf(f, scoreCut=False, requiret0=False, requireCosmic=False, mcs=False, det="SBND", updatecalo = updatecalo, **trkArgs)
